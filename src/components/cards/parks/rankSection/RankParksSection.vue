<!-- urbverde-ui/src/components/cards/parks/rankSection/RankParksSection.vue -->
<template>
    ...
    async fetchRankingData() {
      ...
      try {
        ...
        if (response.status === 200 && Array.isArray(response.data)) {
          ...
          console.warn('Invalid response format from parks ranking API:', response.data);
          this.rankCards = [];
          this.isError = true;
        }
      } catch (error) {
        console.error('Error fetching parks ranking data:', error);
        this.rankCards = [];
        this.isError = true;
      } finally {
